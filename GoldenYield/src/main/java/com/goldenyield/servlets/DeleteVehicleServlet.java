...
public class DeleteVehicleServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		String query2="delete from addvehicle where vehiclesecretid=?";
		String query3="delete from vehicleregistration where vehicleid=?";
		...
		try(Connection conn=DBUtil.getConnection()) {
			...
			if(result.next()) {
				...
				if(update1>0 && update2>0) {
					...
					req.getRequestDispatcher("deletevehicle.jsp").forward(req,resp);
				}
				else {
